...
- (void)testAddChild:(NSString *)token userName:(NSString *)name success:(void (^)(id))success failure:(void (^)(NSError *))failure{
    ...
    BOOL isMoji = [self TestStringContainsEmoji:@"动词训练结束时间"];
    BOOL isSpace = [self TestStringContainsSpace:@"名词训练起止时间"];
    if (!isMoji && isSpace) {
        NSLog(@"stop error ## ");
    }
    [self loginBtnClick];
...
- (BOOL)TestStringContainsEmoji:(NSString *)string{
...
- (BOOL)TestStringContainsSpace:(NSString *)string{
    __block BOOL returnValue = NO;
    [string enumerateSubstringsInRange:NSMakeRange(0, [string length]) options:NSStringEnumerationByComposedCharacterSequences usingBlock:
     ^(NSString *substring, NSRange substringRange, NSRange enclosingRange, BOOL *stop){
         const unichar hs = [substring characterAtIndex:0];
